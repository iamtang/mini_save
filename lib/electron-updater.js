...
class ElectronUpdater {
  ...
  constructor(options = {}) {
    ...
    if (!this.options.owner || !this.options.repo) {
      ...
      if (config) {
        this.options.owner = this.options.owner || config.owner;
        this.options.repo = this.options.repo || config.repo;
      }
    }
    ...
    // 检测是否是开发环境
    this.isDev = !process.resourcesPath;
  }
  ...
  #loadPackageJson(pkgPath = null) {
    try {
      if (pkgPath) {
        this.#pkgCache = JSON.parse(fs.readFileSync(pkgPath, 'utf-8'));
      } else {
        // 尝试多个可能的路径
        const possiblePaths = [
...
`;
  ...
  async checkForUpdates(options = {}) {
    ...
    // 开发环境不检查更新
    if (this.isDev) {
      this.log?.info("开发环境，跳过更新检查");
      return { hasUpdates: false, message: "开发环境，跳过更新检查" };
    }
  ...
  async performUpdate(options = {}) {
    // 开发环境不执行更新
    if (this.isDev) {
      this.log?.info("开发环境，跳过自动更新");
      return { success: false, message: "开发环境，跳过自动更新" };
    }
